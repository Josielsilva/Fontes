object frmOrcamentoCliente: TfrmOrcamentoCliente
  Left = 0
  Top = 0
  BorderStyle = bsDialog
  Caption = 'Relat'#243'rio de or'#231'amento por cliente'
  ClientHeight = 98
  ClientWidth = 271
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 271
    Height = 98
    Align = alClient
    TabOrder = 0
    object btnGravar: TBitBtn
      Left = 8
      Top = 65
      Width = 248
      Height = 25
      Caption = 'OK'
      Glyph.Data = {
        
        D97851D46F55CE7371C986AFC6B3FF00FFFF00FFFF00FFFF00FF}
      TabOrder = 0
      OnClick = btnGravarClick
    end
    object edtClienteInicio: TLabeledEdit
      Left = 8
      Top = 32
      Width = 97
      Height = 21
      EditLabel.Width = 63
      EditLabel.Height = 13
      EditLabel.Caption = 'Cliente Inicial'
      TabOrder = 1
    end
    object EdtClienteFim: TLabeledEdit
      Left = 159
      Top = 32
      Width = 97
      Height = 21
      EditLabel.Width = 58
      EditLabel.Height = 13
      EditLabel.Caption = 'Cliente Final'
      TabOrder = 2
    end
  end
end
