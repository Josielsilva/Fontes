...
begin
  ...
  try
    ...
    Qry.ParamByName('vendaId').AsInteger :=F_OrcamentoId;
    Try
      Qry.ExecSQL;
...
begin
  try
    ...
    Qry.SQL.Add('UPDATE vendas '+
                ...
                ' WHERE vendaId=:vendaId ');
    Qry.ParamByName('vendaId').AsInteger    :=Self.F_OrcamentoId;
    Qry.ParamByName('clienteId').AsInteger  :=Self.F_clienteId;
    Qry.ParamByName('dataVenda').AsDateTime :=Self.F_DataOrcamento;
    Qry.ParamByName('totalVenda').AsFloat   :=Self.F_totalOrcamento;
    ...
    Try
      ...
      cds.First;
      while not cds.Eof do begin
        if EsteItemExiste(Self.F_OrcamentoId, cds.FieldByName('produtoId').AsInteger) then begin
          AtualizarItem(cds);
        end
        else begin
...
Procedure TOrcamento.RetornarEstoque(sCodigo:String; Acao:TAcaoExcluirEstoque);
var Qry:TZQuery;
...
Procedure TOrcamento.BaixarEstoque(produtoId:Integer; Quantidade:Double);
...
function TOrcamento.InserirItens(cds:TClientDataSet; IdVenda:Integer):Boolean;
var Qry:TZQuery;
begin
  try
    Result:=true;
...
begin
  try
    ...
    Try
      ...
      Self.F_OrcamentoId    := Qry.FieldByName('vendaId').AsInteger;
      Self.F_clienteId  := Qry.FieldByName('clienteId').AsInteger;
      Self.F_DataOrcamento  := Qry.FieldByName('dataVenda').AsDateTime;
      Self.F_totalOrcamento := Qry.FieldByName('totalVenda').AsFloat;
...
function TOrcamento.EsteItemExiste(vendaId: Integer; produtoId:Integer): Boolean;
var Qry:TZQuery;
begin
  try
    Qry:=TZQuery.Create(nil);
