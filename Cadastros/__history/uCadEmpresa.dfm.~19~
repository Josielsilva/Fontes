inherited frmCadEmpresa: TfrmCadEmpresa
  Caption = 'Cadastro de Empresa'
  ClientHeight = 387
  Position = poScreenCenter
  ExplicitWidth = 753
  ExplicitHeight = 426
  PixelsPerInch = 96
  TextHeight = 13
  inherited Panel2: TPanel
    Top = 346
    TabOrder = 1
    ExplicitTop = 346
    inherited btnNavegador: TDBNavigator
      Hints.Strings = ()
    end
  end
  inherited Panel3: TPanel
    Height = 346
    TabOrder = 0
    ExplicitHeight = 346
    inherited pgcPadrao: TPageControl
      Height = 346
      ActivePage = TabSheet2
      ExplicitHeight = 346
      inherited TabSheet1: TTabSheet
        ExplicitLeft = 4
        ExplicitTop = 24
        ExplicitWidth = 729
        ExplicitHeight = 318
        inherited dbgPadrao: TDBGrid
          Height = 261
        end
      end
      inherited TabSheet2: TTabSheet
        ExplicitLeft = 4
        ExplicitTop = 24
        ExplicitWidth = 729
        ExplicitHeight = 318
        object Label2: TLabel
          Left = 146
          Top = 17
          Width = 25
          Height = 13
          Caption = 'CNPJ'
        end
        object sbBuscaCep: TSpeedButton
          Left = 116
          Top = 116
          Width = 17
          Height = 21
          Hint = 'Buscar cep'
          Glyph.Data = {
            
            FF00}
          OnClick = sbBuscaCepClick
        end
        object Label5: TLabel
          Left = 7
          Top = 100
          Width = 104
          Height = 13
          Caption = 'Busca cep autom'#225'tico'
        end
        object lblCep: TLabel
          Left = 7
          Top = 182
          Width = 19
          Height = 13
          Caption = 'Cep'
        end
        object Label1: TLabel
          Left = 222
          Top = 182
          Width = 63
          Height = 13
          Caption = 'Telefone fixo'
        end
        object Label3: TLabel
          Left = 389
          Top = 182
          Width = 60
          Height = 13
          Caption = 'Telefone Cel'
        end
        object EdtCodigo: TLabeledEdit
          Tag = 1
          Left = 7
          Top = 32
          Width = 121
          Height = 21
          EditLabel.Width = 92
          EditLabel.Height = 13
          EditLabel.Caption = 'C'#243'digo da empresa'
          TabOrder = 0
        end
        object edtRazao: TLabeledEdit
          Left = 7
          Top = 75
          Width = 363
          Height = 21
          EditLabel.Width = 60
          EditLabel.Height = 13
          EditLabel.Caption = 'Raz'#227'o Social'
          TabOrder = 2
        end
        object edtFantasia: TLabeledEdit
          Left = 376
          Top = 75
          Width = 314
          Height = 21
          EditLabel.Width = 43
          EditLabel.Height = 13
          EditLabel.Caption = 'Fantiasia'
          TabOrder = 3
        end
        object lblEndereco: TLabeledEdit
          Left = 146
          Top = 116
          Width = 469
          Height = 21
          EditLabel.Width = 45
          EditLabel.Height = 13
          EditLabel.Caption = 'Endere'#231'o'
          TabOrder = 5
        end
        object lblNumero: TLabeledEdit
          Left = 621
          Top = 116
          Width = 69
          Height = 21
          EditLabel.Width = 12
          EditLabel.Height = 13
          EditLabel.Caption = 'N'#186
          TabOrder = 6
        end
        object lblBairro: TLabeledEdit
          Left = 7
          Top = 155
          Width = 313
          Height = 21
          EditLabel.Width = 28
          EditLabel.Height = 13
          EditLabel.Caption = 'Bairro'
          TabOrder = 7
        end
        object lblCidade: TLabeledEdit
          Left = 327
          Top = 155
          Width = 322
          Height = 21
          EditLabel.Width = 33
          EditLabel.Height = 13
          EditLabel.Caption = 'Cidade'
          TabOrder = 8
        end
        object edtCEP: TMaskEdit
          Left = 7
          Top = 196
          Width = 197
          Height = 21
          EditMask = '99999-999;1;_'
          MaxLength = 9
          TabOrder = 9
          Text = '     -   '
        end
        object lblUF: TLabeledEdit
          Left = 655
          Top = 155
          Width = 35
          Height = 21
          EditLabel.Width = 13
          EditLabel.Height = 13
          EditLabel.Caption = 'UF'
          MaxLength = 2
          TabOrder = 10
        end
        object edtTelefone: TMaskEdit
          Left = 222
          Top = 196
          Width = 161
          Height = 21
          EditMask = '(99) 9999-9999;1;_'
          MaxLength = 14
          TabOrder = 11
          Text = '(  )     -    '
        end
        object edtTelefoneCel: TMaskEdit
          Left = 391
          Top = 196
          Width = 190
          Height = 21
          EditMask = '(99) 99999-9999;1;_'
          MaxLength = 15
          TabOrder = 12
          Text = '(  )      -    '
        end
        object lblEmail: TLabeledEdit
          Left = 7
          Top = 246
          Width = 350
          Height = 21
          EditLabel.Width = 28
          EditLabel.Height = 13
          EditLabel.Caption = 'E-Mail'
          TabOrder = 13
        end
        object edtIe: TLabeledEdit
          Left = 362
          Top = 246
          Width = 161
          Height = 21
          EditLabel.Width = 87
          EditLabel.Height = 13
          EditLabel.Caption = 'Inscri'#231#227'o estadual'
          TabOrder = 14
        end
        object edtIM: TLabeledEdit
          Left = 529
          Top = 246
          Width = 161
          Height = 21
          EditLabel.Width = 89
          EditLabel.Height = 13
          EditLabel.Caption = 'Inscri'#231#227'o municipal'
          TabOrder = 15
        end
        object edtCNPJ: TMaskEdit
          Left = 143
          Top = 32
          Width = 164
          Height = 21
          EditMask = '##.###.###/####-##'
          MaxLength = 18
          TabOrder = 1
          Text = '  .   .   /    -  '
        end
        object edtBuscaCep: TMaskEdit
          Left = 7
          Top = 116
          Width = 107
          Height = 21
          EditMask = '99999-999;1;_'
          MaxLength = 9
          TabOrder = 4
          Text = '     -   '
        end
        object edtContato: TLabeledEdit
          Left = 7
          Top = 289
          Width = 350
          Height = 21
          EditLabel.Width = 39
          EditLabel.Height = 13
          EditLabel.Caption = 'Contato'
          TabOrder = 16
        end
      end
    end
  end
  inherited zqPrincipal: TZQuery
    Connection = DtmPrincipal.ConexaoDB
    SQL.Strings = (
      'SELECT EMP_COD '
      '       ,EMP_CNPJ'
      '      ,EMP_RAZAOSOCIAL'
      '      ,EMP_FANTASIA'
      '      ,EMP_ENDERECO'
      '      ,Emp_Numero'
      '      ,Emp_Cep'
      '      ,EMP_Bairro'
      '      ,Emp_UF'
      '      ,EMP_Cidade'
      '      ,Emp_Telefone1'
      '      ,Emp_Telefone2'
      '      ,EMP_Email'
      '  FROM Empresa')
    object zqPrincipalEMP_CNPJ: TWideStringField
      DisplayLabel = 'Cnpj'
      FieldName = 'EMP_CNPJ'
    end
    object zqPrincipalEMP_RAZAOSOCIAL: TWideStringField
      DisplayLabel = 'Raz'#227'o social'
      FieldName = 'EMP_RAZAOSOCIAL'
      Size = 100
    end
    object zqPrincipalEMP_FANTASIA: TWideStringField
      DisplayLabel = 'Nome fantasia'
      FieldName = 'EMP_FANTASIA'
      Size = 50
    end
    object zqPrincipalEMP_ENDERECO: TWideStringField
      DisplayLabel = 'Endere'#231'o'
      FieldName = 'EMP_ENDERECO'
      Size = 50
    end
    object zqPrincipalEmp_Numero: TIntegerField
      DisplayLabel = 'N'#186
      FieldName = 'Emp_Numero'
    end
    object zqPrincipalEmp_Cep: TWideStringField
      DisplayLabel = 'Cep'
      FieldName = 'Emp_Cep'
      Size = 9
    end
    object zqPrincipalEMP_Bairro: TWideStringField
      DisplayLabel = 'Bairro'
      FieldName = 'EMP_Bairro'
    end
    object zqPrincipalEmp_UF: TWideStringField
      DisplayLabel = 'UF'
      FieldName = 'Emp_UF'
      Size = 2
    end
    object zqPrincipalEMP_Cidade: TWideStringField
      DisplayLabel = 'Cidade'
      FieldName = 'EMP_Cidade'
    end
    object zqPrincipalEmp_Telefone1: TWideStringField
      DisplayLabel = 'Tel. Fixo'
      FieldName = 'Emp_Telefone1'
      Size = 15
    end
    object zqPrincipalEmp_Telefone2: TWideStringField
      DisplayLabel = 'Tel. Celular'
      FieldName = 'Emp_Telefone2'
      Size = 15
    end
    object zqPrincipalEMP_Email: TWideStringField
      DisplayLabel = 'E-mail'
      FieldName = 'EMP_Email'
      Size = 50
    end
    object zqPrincipalEMP_COD: TIntegerField
      DisplayLabel = 'C'#243'digo'
      FieldName = 'EMP_COD'
    end
  end
  object RESTClient1: TRESTClient
    Accept = 'application/json;q=0.9,text/plain;q=0.9,text/html'
    AcceptCharset = 'UTF-8'
    BaseURL = 'http://viacep.com.br'
    Params = <>
    HandleRedirects = True
    RaiseExceptionOn500 = False
    Left = 368
  end
  object RESTRequest1: TRESTRequest
    Accept = 'application/json;q=0.9,text/plain;q=0.9,text/html'
    AcceptCharset = 'UTF-8'
    Client = RESTClient1
    Params = <
      item
        Kind = pkURLSEGMENT
        name = 'cep'
        Options = [poAutoCreated]
      end
      item
        Kind = pkURLSEGMENT
        name = 'formato'
        Options = [poAutoCreated]
      end>
    Resource = 'ws/{cep}/{formato}'
    Response = RESTResponse1
    Timeout = 0
    SynchronizedEvents = False
    Left = 424
    Top = 32
  end
  object RESTResponse1: TRESTResponse
    Left = 496
    Top = 31
  end
end
